...
function encrypt_secure($value, $action = 'e')
{
  ...
  if ($action === 'd') {
    ...
    if (strlen($data) < ($ivLen + $macLen + 1)) return false;
    ...
    if (!hash_equals($mac, $calcMac)) return false;
...
// $dec= encrypt_secure($_COOKIE['adminuserstart'], $action = 'd' );

// elseif(!empty($_COOKIE['startusuario'])):

//   $dec = encrypt_secure($_COOKIE['startusuario'], $action = 'd');
...
if (!function_exists('auth_parse_id_from_payload')) {
  function auth_parse_id_from_payload(string $dec): int
  {
    ...
    if ($id0 === '' || !ctype_digit($id0)) return 0;
...
if (!function_exists('auth_get_con')) {
  ...
  {
    global $con;
    if (isset($con) && $con instanceof PDO) return $con;
