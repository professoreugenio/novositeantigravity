...
<head>
    ...
    <style>
        ...
        .filter-btn.active { background-color: #0d6efd; color: white; border-color: #0d6efd; box-shadow: 0 4px 6px -1px rgba(13, 110, 253, 0.2); }
        .anexo-item { transition: opacity 0.3s ease, transform 0.3s ease; }
        .anexo-item:hover { background-color: rgba(0,0,0,0.01); }
        .file-icon-box { width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; border-radius: 12px; }
    </style>
</head>
            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-4 list-container mb-5">
                ...
                foreach ($todosAnexos as $anexo): 
                    ...
                    if ($isUrl) {
                        ...
                        if (in_array($ext, ['pdf'])) { $typeGrp = 'pdf'; $iconClass = "bi-file-earmark-pdf-fill text-danger"; $bgColor = "bg-danger"; $textColor = "text-danger"; }
                        elseif (in_array($ext, ['doc', 'docx'])) { $typeGrp = 'doc'; $iconClass = "bi-file-earmark-word-fill text-primary"; $bgColor = "bg-primary"; $textColor = "text-primary"; }
                        elseif (in_array($ext, ['xls', 'xlsx'])) { $typeGrp = 'xls'; $iconClass = "bi-file-earmark-excel-fill text-success"; $bgColor = "bg-success"; $textColor = "text-success"; }
                        elseif (in_array($ext, ['ppt', 'pptx'])) { $typeGrp = 'ppt'; $iconClass = "bi-file-earmark-slides-fill text-warning"; $bgColor = "bg-warning"; $textColor = "text-warning"; }
                        elseif (in_array($ext, ['txt'])) { $typeGrp = 'txt'; $iconClass = "bi-file-earmark-text-fill text-secondary"; $bgColor = "bg-secondary"; $textColor = "text-secondary"; }
                        elseif (in_array($ext, ['zip', 'rar'])) { $typeGrp = 'zip'; $iconClass = "bi-file-earmark-zip-fill text-dark"; $bgColor = "bg-dark"; $textColor = "text-dark"; }
                        elseif (in_array($ext, ['jpg', 'jpeg', 'png', 'gif'])) { $typeGrp = 'img'; $iconClass = "bi-image-fill text-info"; $bgColor = "bg-info"; $textColor = "text-info"; }
                    }
